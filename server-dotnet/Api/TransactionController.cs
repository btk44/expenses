...
public class TransactionController
{
    ...
    public async Task<ActionResult<int>> SearchCount([FromBody] TransactionSearchCommand command){
        var transactionSearchCommandHandler = new TransactionSearchCommandHandler(_dbContext, _mapper);
        return await transactionSearchCommandHandler.HandleCount(command);
    }

    [HttpPost("search-sum")] // not needed for now - consider remove ( to do )
    public async Task<ActionResult<double>> SearchSum([FromBody] TransactionSearchCommand command){
        var transactionSearchCommandHandler = new TransactionSearchCommandHandler(_dbContext, _mapper);
        return await transactionSearchCommandHandler.HandleSum(command);
    }
